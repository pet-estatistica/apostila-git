...
* O asterisco ("*") pode ser usado para subistituir parte do nome de arquivos ou pastas ou o nome inteiro. Isto é valido também para a extensão do arquivo.

* Para ignorar um arquivo, ele pode ser adicionado com seu nome e extensão a lista, com um "*" substituindo o nome ou a extensão, ou substituindo âmbos.

* Um diretório pode ser ignorado adicionando seu nome e uma barra invertida ao final deste.
...
É preciso copiar o arquivo `.gitignore` da pasta do projeto para a pasta 
de usuário. Isso pode ser feito usando-se o comando abaixo:

```{r, engine="bash", echo=-c(1:2)}
cp ~/workspace/meupc/meu1repo/.gitignore ~/
```

Depois de efetuar a cópia, é preciso marcar seu arquivo como global,
adicionando-o no `.gitconfig`. Isso pode ser feito de forma automatica 
usando-se o comando abaixo:

```{r, engine="bash", echo=-c(1:2)}
git config --global core.excludesfile ~/.gitignore
```

 Depois de efetuar esses dois passos, seu `.gitignore` estará como global.
 
 Uma observação importante a ser feita é que, se adicionado como global 
 uma exclusão a arquivos PDF (Usando *.pdf), todos os arquivos com esta 
 extensão serão ignorados pelo git, não somente o do projeto atual. 
 Postanto, é recomendado somente adicionar globalmente arquivos auxiliares,
 que não são necessários para compilação.
